...
public class addActivity2 extends AppCompatActivity  {
    ...
    private int counter = 0; // Tracks dynamic image index
    private GridLayout gridLayout;

    // Fixed references for first 6 (optional, but kept for compatibility)
    ...
    // Intent data
    String roomlocation, district, institution, roomname, roomcap;
    String latitude, longitude, bookedbygender, roomprice, roomType, rules, facilities, roomcapacityreached;
    ...
    private void applySystemBarPadding() {
        ...
        NestedScrollView scrollView = findViewById(R.id.main2);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupImagePicker();
        setupCameraLauncher();
        initializeInitialGrid(); // Create first 6 programmatically
    ...
    private void initializeInitialGrid() {
        counter = 0;
        addInitialRow(0); // Indices 1,2,3
        addInitialRow(1); // Indices 4,5,6

        // Find views by ID after creation
        ...
        setupFixedClickListeners();
    }

    private void addInitialRow(int rowOffset) {
        int startIndex = rowOffset * 3 + 1;
        List<ImageView> imageViews = new ArrayList<>();
        List<Button> buttons = new ArrayList<>();

        float scale = getResources().getDisplayMetrics().density;
        int heightInPixels = (int) (150 * scale + 0.5f);
        int paddingInPixels = (int) (16 * scale + 0.5f);
        gridLayout.setPadding(paddingInPixels, paddingInPixels, paddingInPixels, paddingInPixels);

        // Add 3 ImageViews
        for (int i = 0; i < 3; i++) {
            int index = startIndex + i;
            String imageIdName = index <= 3 ? "roompic" + index : "roompic" + (index == 4 ? "111" : index == 5 ? "222" : "333");
            int resId = getResources().getIdentifier(imageIdName, "id", getPackageName());

            GridLayout.LayoutParams lp = new GridLayout.LayoutParams();
            lp.rowSpec = GridLayout.spec(GridLayout.UNDEFINED);
            lp.columnSpec = GridLayout.spec(i, 1f);
            lp.width = 0;
            lp.height = heightInPixels;
            ...
            iv.setImageResource(R.drawable.roompic);
        ...
        for (int i = 0; i < 3; i++) {
            int index = startIndex + i;
            String buttonIdName = index <= 3 ? "roompic" + index + "button" : "roompic" + (index == 4 ? "111" : index == 5 ? "222" : "333") + "button";
            int resId = getResources().getIdentifier(buttonIdName, "id", getPackageName());
            ...
            btn.setText("Choose");
            btn.setBackgroundResource(R.drawable.fancy_button_background);
    ...
    private void setupFixedClickListeners() {
        if (rpic1 != null) rpic1.setOnClickListener(v -> { currentSelectedImageView = roompic1; currentImageIndex = 1; showImagePickerDialog(); });
        if (rpic2 != null) rpic2.setOnClickListener(v -> { currentSelectedImageView = roompic2; currentImageIndex = 2; showImagePickerDialog(); });
        if (rpic3 != null) rpic3.setOnClickListener(v -> { currentSelectedImageView = roompic3; currentImageIndex = 3; showImagePickerDialog(); });
        if (rpic4 != null) rpic4.setOnClickListener(v -> { currentSelectedImageView = roompic4; currentImageIndex = 4; showImagePickerDialog(); });
        if (rpic5 != null) rpic5.setOnClickListener(v -> { currentSelectedImageView = roompic5; currentImageIndex = 5; showImagePickerDialog(); });
        if (rpic6 != null) rpic6.setOnClickListener(v -> { currentSelectedImageView = roompic6; currentImageIndex = 6; showImagePickerDialog(); });
    }
    ...
    private void addRowToGrid(GridLayout gridLayout, String name, String id) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            iv.setTag(counter + i + 1);
            iv.setLayoutParams(lp);
            iv.setImageResource(R.drawable.roompic);
            iv.setAdjustViewBounds(true);
            iv.setScaleType(ImageView.ScaleType.CENTER_CROP);
            gridLayout.addView(iv);
            newImageViews.add(iv);
        }
        ...
        for (int i = 0; i < 3; i++) {
            final ImageView iv = newImageViews.get(i);
            final Button btn = newButtons.get(i);
    ...
    private void setupCameraLauncher() {
        cameraLauncher = registerForActivityResult(
                ...
                result -> {
                    if (result && currentPhotoUri != null && currentSelectedImageView != null) {
                        Dialog progressDialog = new Dialog(addActivity2.this);
                        View view = LayoutInflater.from(addActivity2.this)
                                .inflate(R.layout.progress_dialog_layout, null);
                        progressDialog.setContentView(view);
                        ...
                        currentSelectedImageView.setImageURI(currentPhotoUri);
                        loadAndCompressImage(currentPhotoUri, currentImageIndex, progressDialog);
                    } else {
                ...
                .into(new CustomTarget<Bitmap>() {
                    ...
                    public void onResourceReady(Bitmap resource, Transition<? super Bitmap> transition) {
                        ...
                        if (compressedData.length / 1024 < 5) {
                            ...
                            Toast.makeText(addActivity2.this, "Image too small, try larger picture", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        ...
                        ds.putBytes(compressedData)
                                .addOnSuccessListener(taskSnapshot -> {
                                    imagecount++;
                                    progressDialog1.dismiss();
                                    Toast.makeText(addActivity2.this, "Image uploaded", Toast.LENGTH_SHORT).show();
                                })
                                .addOnFailureListener(e -> {
                                    progressDialog1.dismiss();
                                    Toast.makeText(addActivity2.this, "Upload failed", Toast.LENGTH_SHORT).show();
                                });
                    }
    ...
    public void submit(View view) {
        ...
        /* if (imagecount >= 6) {
            ...
            Toast.makeText(this, "Upload at least 6 images", Toast.LENGTH_LONG).show();
        }*/
    }
    @Override
            public void onPause()
    ...
    public void onResume()
    {
        ...
        if(ss!=null)
        {
            du.addValueEventListener(ss);
        }
    }
ValueEventListener ss;
    ...
    private void updateDatabase() {
       ...
       du=FirebaseDatabase.getInstance().getReference("google").child(name1).child(id1).child("owner").child("doneuploading");
      ss=new ValueEventListener() {
          @Override
          public void onDataChange(@NonNull DataSnapshot snapshot) {
...
Log.d("here","here");
                ...
                ownerData.put("roomcapacity", roomcapacity);
                ...
                ownerData.put("capacityremaining",roomcapacity );
                ...
                currentData.child("room" +id1+d).setValue(ownerData);
...
//                ownerData.put("facilities", facilities);
//                ownerData.put("roomcapacityreached", capacityofroomreached);
//                ownerData.put("capacityremaining",0);
//                ownerData.put("ownername", name1);
//                ownerData.put("ownerid", id1);
//                ownerData.put("roomsexplored", sss);
//                ownerData.put("roomsposted", ddd);
